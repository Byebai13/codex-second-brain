...
project rules, user preferences, core viewpoints, confirmed decisions, reusable
workflows, source notes, candidate memories, and conflicts into the right files.
...
如果没有合适的记忆区，请创建或补齐 codex-brain/，并把项目规则、用户偏好、核心观点、已确认决策、可复用工作流、资料卡片、候选记忆和冲突更新分别放到正确文件里。
...
```text
Please do a second-brain closing sweep: decide whether anything from this task should become a candidate memory, preference, core viewpoint, decision, workflow, source note, or conflict update, then update the relevant files and report the changes.
```
